...
raw_SS_result_folder = '20231214-105413_S52512_SS'
    ...
    for case in cases:
        for name,sheet in sheets_dict.items():
            if name == 'Voltage Level'+ case + genoff:
                ...
                result_dfs['df_line_ldng' + case+genoff] = pd.DataFrame(data=sheet, columns = ['CaseNr','Case_Code','brch_name','Loading (%)']).round(3)
                result_dfs['df_line_ldng' + case+genoff].rename(columns = {'CaseNr':'Case Number','Case_Code': 'Case Reference','brch_name':'Branch Name','Loading (%)': 'Loading(%) GenOFF'}, inplace = True)
            elif name == 'Line Loadings'+case+genon:
                 result_dfs['df_line_ldng' + case+genon] = pd.DataFrame(data=sheet, columns = ['CaseNr','Case_Code','brch_name','Loading (%)']).round(3)
                 result_dfs['df_line_ldng' + case+genon].rename(columns = {'CaseNr':'Case Number','Case_Code': 'Case Reference','brch_name':'Branch Name','Loading (%)': 'Loading(%) GenON'}, inplace = True)
            elif name == 'Volt Fluc GenChg'+case+genon:
                result_dfs['df_vltfluc_genchg'+case+genon] = pd.DataFrame(data=sheet, columns = ['CaseNr','Case_Code','bus_name','PU','PU_final','VolDev (%)']).round(3)
                result_dfs['df_vltfluc_genchg'+case+genon].rename(columns = {'CaseNr':'Case Number','Case_Code': 'Case Reference','bus_name':'Bus Name','PU':'Voltage Level(pu) Prior','PU_final': 'Voltage Level(pu) After','VolDev (%)': 'Volt Fluc(%)'}, inplace = True)
            elif name == 'Volt Fluc Lol'+case+genon:
                result_dfs['df_vltfluc_lol'+case+genon] = pd.DataFrame(data=sheet, columns = ['CaseNr','Case_Code','bus_name','PU','VolDev (%)']).round(3)
                result_dfs['df_vltfluc_lol'+case+genon].rename(columns = {'CaseNr':'Case Number','Case_Code': 'Case Reference','bus_name':'Bus Name','PU':'Voltage Level(pu) GenON','VolDev (%)': 'Volt Fluc(%) GenON'}, inplace = True)
            elif name == 'Volt Fluc Lol'+case+genoff:
                result_dfs['df_vltfluc_lol'+case+genoff] = pd.DataFrame(data=sheet, columns = ['CaseNr','Case_Code','bus_name','PU','VolDev (%)']).round(3)
                result_dfs['df_vltfluc_lol'+case+genoff].rename(columns = {'CaseNr':'Case Number','Case_Code': 'Case Reference','bus_name':'Bus Name','PU':'Voltage Level(pu) GenOFF','VolDev (%)': 'Volt Fluc(%) GenOFF'}, inplace = True)
...
#def generate_plot(x_axis, [y_axis1, y_axis2], [legend1, legend2], label_x, label_y, title):
...
def sumarise_results(result_dfs, summary_dfs):
    ...
    for case in summary_dfs.keys():
#        case_dict = {'voltage_levels': 'df_vltg_lvl_'+case}
        ...
        for key,value in result_dfs.items():
            if 'df_vltg_lvl_'+case+'_off' in key: # summarise the voltage level results with and without plant
                #Summary table
            ...
            elif 'df_line_ldng_'+case+'_off' in key: # summarise the line loading results for network normal and contingencies
            ...
            elif 'df_vltfluc_genchg_'+case+'_on' in key: # summary tables for voltage fluctuation due to change in generation
            ...
            elif 'df_vltfluc_lol_'+case+'_on' in key: # summary tables for voltage fluctuation due to loss of a line
            ...
            elif 'df_fault_lvl_' + case + '_on' in key: #  populate the summary for fault level results
                #summary table
...
# summarise the voltage level results with and without plant
...
# summarise the line loading results for network normal and contingencies
...
# summary tables for voltage fluctuation due to change in generation
...
# summary tables for voltage fluctuation due to loss of a line
