...
datasets_PSSE = {'label':'PSSE_Data', 'path':r"PSSE_sim\result_data\dynamic_smib\20240304-2111_gps_dbg", 'ID': 0, 'timeID':'Time(s)', 'timeoffset':-3.0,
...
datasets_PSCAD = {'label':'PSCAD_Data', 'path':r"PSCAD_sim\result_data\dynamic_smib\20240304-1612_gps_dbg", 'ID': 4, 'timeID':'time(s)', 'timeoffset':-3.0, 
...
datasets_nw_genon = {'label':'PSSE_genon', 'path':r"PSSE_sim\result_data\dynamic_network\Final Results GPS NEM genon", 'ID': 2, 'timeID':'Time(s)', 'timeoffset':-2.0,
                  'calcCurrents':[{"P":"P_POC1", "Q":"Q_POC1", "V":"U_POC1", "nameLabel":"PLANT", "scaling":-0.010334129, }, # #negative due to the reversed Q measurement
                                  {"P":"P_LV1", "Q":"Q_LV1", "V":"U_LV1", "nameLabel":"PV", "scaling":0.0099206, }, #1/Sbase_POC for Iq at POC or 1/Sbase_INV for Iq at INV -> convert QMVAr to Qpu for calculation
                                  {"P":"P_LV2", "Q":"Q_LV2", "V":"U_LV2", "nameLabel":"BESS", "scaling":0.010334129, },], }
...
datasets = [datasets_PSSE,datasets_PSCAD] # [datasets_PSSE, datasets_PSCAD,datasets_nw_genon,datasets_nw_genoff]
...
channels_lib = { #to be used in the ploting each graph below
                ...
                'NW_pre':   {'V_POC':{'dataset':2, 'name':"U_POC1", 'leg':'pre-SUMSF', 'offset':0.0, 'scale':1.0},
                             'Lismore330':{'dataset':2, 'name':"V_LSM_330", 'leg':'pre-SUMSF', 'offset':0.0, 'scale':1.0},
                             'Coffs_Harbour330':{'dataset':2, 'name':"V_COFF_330", 'leg':'pre-SUMSF', 'offset':0.0, 'scale':1.0},
                             'Lismore132':{'dataset':2, 'name':"V_LSM_132", 'leg':'pre-SUMSF', 'offset':0.0, 'scale':1.0},
                             'Koolkhan132':{'dataset':2, 'name':"V_KOLK_132", 'leg':'pre-SUMSF', 'offset':0.0, 'scale':1.0},
                             },
...
#                    row_cells[7].text=str(test_details['setpoint']['GridMVA'])
    ...
    if(any('tov' in case for case in cases)):
        p.add_run('Table '+str(tableCnt)+': Scenario list - Temporary Over-Voltage tests').bold=True        
        headers=['Case Nr.', 'Test Type', 'Time', 'TOV duration', 'Capacity(uF)', 'V residual', 'POC voltage', 'Grid SCR', 'Grid X/R', 'Grid SCR post-TOV','Grid X/R post-TOV', 'P at POC (MW)', 'Q at POC (MVAr)', 'Control mode', 'passed']
        table1=report.add_table(rows=1, cols=len(headers))
        table1.style='ListTable3-Accent3'
        hdr_cells1=table1.rows[0].cells
        for header_id in range(0, len(headers)): hdr_cells1[header_id].text=headers[header_id]
