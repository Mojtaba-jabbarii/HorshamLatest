...
def add_conclusion(report, probs):
    ...
    if not flawless:
        ...
        temp_text+="No issues have been detected in the PQ analysis, which may indicate that the proposed location on the network is suitable to host a" + str(ProjectDetailsDict['PlantMW']) + "MW generator without any further limitations from a grid perspective. "
...
def add_appendices(report,summary_dfs):
    ...
    for case in summary_dfs.keys():
        if(summary_dfs[case][Vcp[0]]!={}):
            report.add_heading(ident_case_name(case), level=4)#maybe make this a label of the table instead of making it a  heading.
            for frame_id in range(0, len(summary_dfs[case][Vcp[0]]['appendix'])):#add summary tables of results to word doc.
                data_frame_to_docx_table(summary_dfs[case][Vcp[0]]['appendix'][frame_id], report)
                report.add_paragraph('')
...
#    genon = '_on'
    ...
    for case in cases:
        for name,sheet in sheets_dict.items():
            vol_txt = name[-3:]
            if name == case:
                result_dfs[case] = pd.DataFrame(data=sheet, columns=['1 V_Inv','1 V_Inv2','2 P_Inv','2 P_Inv2','3 Q_Inv','3 Q_Inv2','7 Q_Poc','6 P_Poc']).round(3)
    ...
    for case in summary_dfs.keys():
        ...
        for key,value in result_dfs.items():
            if case in key: # summarise the voltage level results with and without plant
                ...
                df_pq_curve = result_dfs[df_name+'_0.9']
...
#                pvt_df.style.applymap(hl_vltg_lvls_violation,subset = ['Voltage Level(pu) GenOFF','Voltage Level(pu) GenON']).format({'Voltage Level(pu) GenOFF':'{0:,.3f}','Voltage Level(pu) GenON':'{0:,.3f}'})
                summary_dfs[case][Vcp[0]]['summary'] = []
                summary_dfs[case][Vcp[0]]['summary'].append(df_pq_curve)
...
#                imgdata = generate_plot(df_pq_curve['Q_Poc [MVAr]'], [df_pq_curve['P_Poc [MW]']], ['PQ curve'], 'Bus Names', 'Voltage(pu)', 'Voltage Levels')
                ...
                summary_dfs[case][Vcp[0]]['plot'] = []
                summary_dfs[case][Vcp[0]]['plot'].append(imgdata)
...
#                if not vl_violation.empty: vl_violation['Pass'] = vl_violation.apply(vl_check, axis = 1)
                summary_dfs[case][Vcp[0]]['violations'] = []
#                summary_dfs[case][Vcp[0]]['violations'].append(vl_violation)
                
                #Empty apendix
                summary_dfs[case][Vcp[0]]['appendix'] = []
