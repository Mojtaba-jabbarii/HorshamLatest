...
#        BranchLibDict=dict(zip(BranchLibSheet.brch_from, BranchLibSheet.brch_to, BranchLibSheet.brch_id, BranchLibSheet.brch_name, BranchLibSheet.brch_code))
    ...
    #-------------------------------------------------------------------------
    #PowerFactory-SPECIFIC SETTINGS
    if('ModelDetailsPF' in relevant_tabs or relevant_tabs=='all'):  
        PFmodelSheet=pd.read_excel(testdefSheetPath, sheet_name="ModelDetailsPF", usecols="A,B", keep_default_na=False)
        PFmodelDict=dict(zip(PFmodelSheet.ParamIdentifier, PFmodelSheet.Value))
        if('' in PFmodelSheet.keys()):
            del PFmodelDict['']
        #print(FileSettingsDict)
        print("Read PowerFactory model settings")
        return_dict['ModelDetailsPF']=PFmodelDict
        
    #-------------------------------------------------------------------------
    if('PowerQuality' in relevant_tabs or relevant_tabs=='all'):
        PowerQualityDict={}
        PowerQualitySheet=pd.read_excel(testdefSheetPath, sheet_name="PowerQuality", usecols=None, keep_default_na=False, skiprows=1)
        for row_cnt in range(0, len(PowerQualitySheet)):
            CaseNr=str(PowerQualitySheet['CaseNr'].iloc[row_cnt])
            if CaseNr != '':
                scenario_name='PQ'+str(CaseNr)
                PowerQualityDict[scenario_name]=[{}]
                for column_name in PowerQualitySheet.columns:
                    if(column_name != 'CaseNr'):
                        PowerQualityDict[scenario_name][0][column_name]=PowerQualitySheet[column_name].iloc[row_cnt]    
            else:
                tempDict={}
                for column_name in PowerQualitySheet.columns:
                    if(column_name != 'CaseNr'):
                        tempDict[column_name]=PowerQualitySheet[column_name].iloc[row_cnt]   
                
                PowerQualityDict[scenario_name].append(tempDict)
        
        print("Read Scenarios - PowerQuality")
        return_dict['PowerQuality']=PowerQualityDict
    #-------------------------------------------------------------------------
    if('PQ Limits' in relevant_tabs or relevant_tabs=='all'):
        #MonitorBuses
        PQ_LimitsSheet=pd.read_excel(testdefSheetPath, sheet_name="PQ Limits", usecols="A,B,C,D,E,F,G", keep_default_na=False, skiprows=1)
        Planning_LimitsDict=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Planning_Limits))
        Allocation_Limits_AAS_Dict=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Allocation_Limits_AAS))
        Allocation_Limits_MAS_Dict=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Allocation_Limits_MAS))
        Bkg_Harm=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Background_Harmonic_Distortion))
        Alpha_Factors=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Alpha_Factors))
        Pass_Fail_Criteria=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Pass_Fail_Criteria))
        #PQ_LimitsDict=dict(zip(PQ_LimitsSheet.Harmonic_Order, PQ_LimitsSheet.Planning_Limits, PQ_LimitsSheet.Allocation_Limits_AAS, PQ_LimitsSheet.Allocation_Limits_MAS))
        if('' in Planning_LimitsDict.keys()):
            del Planning_LimitsDict['']
        if('' in Allocation_Limits_AAS_Dict.keys()):
            del Allocation_Limits_AAS_Dict['']
        if('' in Allocation_Limits_MAS_Dict.keys()):
            del Allocation_Limits_MAS_Dict['']
        if('' in Bkg_Harm.keys()):
            del Bkg_Harm['']
        if('THD' in Alpha_Factors.keys()):
            del Alpha_Factors['THD']
        if('THD' in Pass_Fail_Criteria.keys()):
            del Pass_Fail_Criteria['THD']    
            
        print("Read PQ Limits")
        return_dict['Planning Limits']=Planning_LimitsDict
        return_dict['Allocation Limits_AAS']=Allocation_Limits_AAS_Dict
        return_dict['Allocation Limits_MAS']=Allocation_Limits_MAS_Dict
        return_dict['Bkg_Harm']=Bkg_Harm
        return_dict['Alpha Factors']=Alpha_Factors
        return_dict['Pass_Fail_Criteria']=Pass_Fail_Criteria
    #-------------------------------------------------------------------------
