...
Functionality:
    ...
    4. Gather results pre- and post-connection
...
simulation_batch_label='PQ_with filters'
...
ModelCopyDir=ModelCopyDir0+"\\PQ"
make_dir(ModelCopyDir)
ModelCopyPath=ModelCopyDir+"\\"+testRun

# Directory to store model workspaces copies
wrkg_spc = createPath(ModelCopyDir+"\\"+testRun)
...
Pass_Fail_Criteria = return_dict['Pass_Fail_Criteria']
...
dpl_QatNight = app.GetProjectFolder("script").GetContents(ModelDetailsPF['DPL_Q at Night Characteristic'] + ".ComDpl")[0]
dpl_SwOffHFFdr = app.GetProjectFolder("script").GetContents(ModelDetailsPF['DPL_Switch off HF Feeder'] + ".ComDpl")[0]
dpl_SwOnHFFdr = app.GetProjectFolder("script").GetContents(ModelDetailsPF['DPL_Switch on HF Feeder'] + ".ComDpl")[0]
dpl_InvDispatch = app.GetProjectFolder("script").GetContents(ModelDetailsPF['DPL_Inverter Dispatch'] + ".ComDpl")[0]

def restart_HFtolerances():
    #Deactivate all active filter variations,
    ActiveHFtolerance = app.GetActiveNetworkVariations()
    for i in range(len(ActiveHFtolerance)):
        x = ActiveHFtolerance[i]
        x.Deactivate()
...
for case_num in active_scenarios:
    ...
    #Activate scenario, inverter characteristics, inverter dispatch,  and variation in PowerFactory
    PFactivescen = app.GetProjectFolder("scen").GetContents(scenario[0]['PowerFactory Operation Scenario']+".IntScenario")[0]
    ...
    #Deactivate all active filter variations,
    restart_HFtolerances()
        
    # Activate Filter Variation (only applicable during Filter Design stage, for component tolerance analysis)    
    if scenario[0]['Filter Tolerance']=='':
        pass
    else:
        PFActiveHFTolerance = app.GetProjectFolder("scheme").GetContents(scenario[0]['Filter Tolerance']+".IntScheme")[0]
        PFActiveHFTolerance.Activate()
    ...
    if PQDict[case_num][0]['Inverter Characteristic']=='Q at Night':
        dpl_QatNight.Execute()    
   
    
    
    #Switch on/off harmonic filters depending on the case
    if PQDict[case_num][0]['With Filters'] == 'Yes':
        dpl_SwOnHFFdr.Execute()
    if PQDict[case_num][0]['With Filters'] == 'No':
        dpl_SwOffHFFdr.Execute()     
   
    
    #Set individual inverter dispatch in relation to POC export/import
    dpl_InvDispatch.SetInputParameterDouble('PoC_MW',float(PQDict[case_num][0]['POC MW']))
    dpl_InvDispatch.SetInputParameterDouble('PoC_MVar',float(PQDict[case_num][0]['POC Mvar']))
    dpl_InvDispatch.SetInputParameterDouble('BESS_P_pu',float(PQDict[case_num][0]['BESS_P_pu']))   
    dpl_InvDispatch.SetInputParameterDouble('BESS_Q_pu',float(PQDict[case_num][0]['BESS_Q_pu']))
    dpl_InvDispatch.Execute()
    ...
    #Ensure all frequencies are considered in harmonic calculations
    ...
    #Get HD at each harmonic level and compare with Pass/Fail Criteria
    Activecase_HD = pd.read_csv(outputResultPath + "\\" + case_num + "_Zequiv" + ".csv")
    for i in range(len(Pass_Fail_Criteria)):
        HD_diff = Activecase_HD["HD in %"][i+1] - Pass_Fail_Criteria[i+2]
        harmonic_order = i+2
        if HD_diff>0:
            print("Limit exceeded on " + case_num + " for harmonic order: " + str(harmonic_order) + ", by: " + str(HD_diff))
        else:
            pass
    

    #Deactivate project, then save a copy of the model
    proj = app.GetActiveProject()
    proj.Deactivate()
    currentuser = app.GetCurrentUser()
    ComPfdexport = app.GetCurrentUser().CreateObject("ComPfdexport")
    ComPfdexport.g_objects = [proj]
    ComPfdexport.g_file = ModelCopyPath + "\\" + ModelDetailsPF['Project Name'] + "_" + case_num +".pfd"
    ComPfdexport.Execute()
    
    #Reactivate project and restart all changes
    ...
    print('Elapsed time after previous case: {}'.format(end_time - start_time))
