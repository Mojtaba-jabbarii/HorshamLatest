...
Pmax_loop = Pmax_POC + 1.5 # account for the losses from POC to INV terminal
...
psspy.case(base_model_workspace+"\\" + case_name)
sav_file_name = os.path.basename(base_model_workspace +"\\"+ case_name) # get the file name
...
out_file_name = "S5251_PQ curve results.xlsx" # for recording the results
#out_file_name = "S5251_PQ curve_" +str(source_type)+".xlsx" # for recording the results
#out_file_name = "S5251_" +str(temp_name)+str(round(PV_INV_num,0))+str(round(BESS_INV_num,0))+".xlsx" # for recording the results
writer = pd.ExcelWriter(outputResultPath+'\\'+out_file_name,engine = 'xlsxwriter')
    

for tmp in range(len(temp_cases)): # loop through different temperature cases
    temperature = temp_cases[tmp]
...
#    Ploop_range =  [90, 91, 92]    
    ...
    #                if P_BESS > Sbase_analysis_BESS: P_BESS = Sbase_analysis_BESS
            ...
            for Vset_inv in [1.2, 0.7]: # change voltage setpoint of generator to get a Q range. For each case, solve the case with the tap locked
                ...
                if source_type == "PV":   
                    ...
                    case_text = srce_text+'_'+mode_text+'_'+temp_text
    ...
    #                            elif P_PV < 0: P_PV += P_step
                ...
                ###################################################################
                    ...
                    profile_90_BESS = DeratingDict[case_text+'_0.9']
                    profile_100_BESS = DeratingDict[case_text+'_1.0']
                    profile_110_BESS = DeratingDict[case_text+'_1.1']
...
#    
                    ...
                    while (delta_Vinv > tolerance) and iters_out < 100: # if the voltage level at inveter varies more than the accepted band
                        ...
                        Sbase_PV = Slim_PV_ind * PV_INV_num # Maximum Slim of all INV
                        if Sbase_PV > abs(P_PV): Qlim_PV = math.sqrt(Sbase_PV**2 - P_PV**2) 
                        else: # not engough active power provied -> need to set Q to zero and get other source of power complemented
                            Qlim_PV = 0
#                            delta_P_needed = abs(P_PV) - Sbase_PV
#                            P_BESS += delta_P_needed
#                            Qlim_BESS = math.sqrt(Sbase_BESS**2 - P_BESS**2)
...
#                            Qlim_BESS = math.sqrt(Sbase_BESS**2 - P_BESS**2)
        ...
        #                            if P_BESS < -Sbase_BESS: P_BESS = -Sbase_BESS # Limit P to Sbase
                            ...
                            if (Sinv_PV > Sbase_PV): # If PV overloaded
                                ...
                                if Qlim_PV < 0.0: 
                                    Qlim_PV = 0.0
                                    if P_BESS < 0: # if active power from BESS is negative, then consider to reduce active power from PV
                                        if P_PV > 0: P_PV -= P_step
                                        if P_PV < 0: P_PV = 0 # PV do not absorb P
                                print '*********************************************************'
                                print 'Apparent power limit reached %1.4f, QlimPV reduced to %1.4f' % (Sinv_PV, Qlim_PV)
