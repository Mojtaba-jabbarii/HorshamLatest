...
for temperature in temp_cases: # loop through different temperature cases
    ...
    #            if P_BESS != P_BESS_ori: print 'Contribution from BESS is updated to %1.2f' % (P_BESS/P) # if the proportion from BESS changes, note it down
                ...
                # loop the inverter terminal to make sure the PQS are updated correctly
                iters_out = 0
                delta_Vinv = 1
                while (delta_Vinv > del_Vinv_tol) and iters_out < 100: # if the voltage level at inveter varies more than the accepted band
                    # fetch the terminal voltage for estimating the derating
                    ierr, Vinv_PV_ori = psspy.busdat(busgen_V_PV, 'PU')
                    ierr, Vinv_BESS_ori = psspy.busdat(busgen_V_BESS, 'PU')
                    ...
                    Slim_BESS_ind = apply_derating(P_individual_BESS,Vinv_BESS_ori, case_text)
                    ...
                    Slim_PV_ind = apply_derating(P_individual_PV,Vinv_PV_ori, case_text)
                    Sbase_PV = Slim_PV_ind * PV_INV_num # Maximum Slim of all INV
                    if Sbase_PV > abs(P_PV): Qlim_PV = math.sqrt(Sbase_PV**2 - P_PV**2) 
                    else:Qlim_PV = 0 # not engough active power provied -> need to set Q to zero and get other source of power complemented
                    ...
                    while ((Sinv_BESS > Sbase_BESS+0.001) or (Sinv_PV > Sbase_PV+0.001)) and iters < 10:
                    ...
                    # measuring the terminal voltage after the base is updated
                    ierr, Vinv_PV_update = psspy.busdat(busgen_V_PV, 'PU')
                    ierr, Vinv_BESS_update = psspy.busdat(busgen_V_BESS, 'PU')
                    
                    # calculate the looping condition again
                    time.sleep(0)
#                    ierr, Vinv_BESS = psspy.busdat(busgen_V_BESS, 'PU')
                    delta_Vinv_BESS = abs(Vinv_BESS_update - Vinv_BESS_ori)
#                    ierr, Vinv_PV = psspy.busdat(busgen_V_PV, 'PU')
                    delta_Vinv_PV = abs(Vinv_PV_update - Vinv_PV_ori)                
