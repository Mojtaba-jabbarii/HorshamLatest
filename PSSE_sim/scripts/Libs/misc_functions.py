...
#        for i in range(0,15): #If multiple generator participate in the QV droop, then repeat the process to make sure the actual voltage settle well
        while abs(delta_q_loop) > tol_q and iter_num > 0:
            ...
            if len(gens_with_vdc['gens']) != 0:
                for gen in gens_with_vdc['gens']: # One generator controlling POC voltage
                    ierr,ival = psspy.macint(gen['gen_bus'],gen['gen_id'],'STATUS')
                    ierr,ival2 = psspy.busint(gen['gen_bus'],'TYPE')
                    if ival == 0 or ival2 == 4:
                        print('GEN '+gen['gen_id']+' at bus '+str(gen['gen_bus'])+' is OFF')
                    else:
                        ...
                        try: q_poc_req = gen['q_poc_req']
                        except: q_poc_req = -((gen['poc_q_max']/(gen['poc_drp_pct']/100.0)) * delta_v) # Consider Qmax as the base for droop
