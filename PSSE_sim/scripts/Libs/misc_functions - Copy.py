...
# Initialise gens with voltage droop control mode - multiple generator share one droop characteristic:
def init_gens_vdc2(gens_with_vdc2, err_code = 0, auto_script = ''): 
    if len(gens_with_vdc2['gens']) != 0:
        tol_q = 0.0001 #MVAr
        k_factor = 0.20 # regression factor
        iter_num = 20
        delta_q_loop = 2.0
        q_gen_pct = [0.5,0.5] #reactive power contribution from each generator in percentage
#        for i in range(0,15): #If multiple generator participate in the QV droop, then repeat the process to make sure the actual voltage settle well
        while abs(delta_q_loop) > tol_q and iter_num > 0:
            delta_q_loop = 0
            for gen in gens_with_vdc2['gens']:
...
#                    iter_num = 15
                    ...
                    q_gen1_new = ((q_gen1+q_gen2) + delta_q*k_factor)*q_gen_pct[0]
                    q_gen2_new = ((q_gen1+q_gen2) + delta_q*k_factor)*q_gen_pct[1]
                    ...
                    auto_script+="psspy.machine_chng_2(" + str(gen['gen_bus'][0])+",'1',[_i,_i,_i,_i,_i,_i],[_f,_f,"+str(q_gen1_new)+","+str(q_gen1_new)+",_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f])\n"
                    auto_script+="psspy.machine_chng_2(" + str(gen['gen_bus'][1])+",'1',[_i,_i,_i,_i,_i,_i],[_f,_f,"+str(q_gen2_new)+","+str(q_gen2_new)+",_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f])\n"
                    psspy.fnsl([1,0,0,1,1,0,0,0])
                    psspy.fnsl([1,0,0,1,1,0,0,0])
