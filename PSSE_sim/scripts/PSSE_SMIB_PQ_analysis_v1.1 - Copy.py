...
BESS_pct = 0.0 #[0.0-1.0]BESS setpoint to be a proportion of POC active power setpoint
...
capbanks_off = []
capbanks_off = [
#        {"bus": 334091, "id": "2"}, # for PV 2MVAr -> uncoment the line to turn this off when PV is off
#        {"bus": 334091, "id": "4"}, # for PV 6MVAr -> uncoment the line to turn this off when PV is off
#        {"bus": 334091, "id": "1"}, # for BESS 1MVAr -> uncoment the line to turn this off when BESS is off
#        {"bus": 334091, "id": "3"}, # for BESS 3MVAr -> uncoment the line to turn this off when BESS is off
        ]

# Buses to be deactivated/disconnected
bus_dis_list = []
...
turn_off_capbank(capbanks_off)

for bus_gen_i in bus_gen_list:#set generators to control voltage to 1pu
    psspy.plant_data_4(bus_gen_i,0,[_i,_i],[ 1.0,_f])
#psspy.plant_data_4(273501,0,[_i,_i],[ 1.0,_f])
#psspy.plant_data_4(273502,0,[_i,_i],[ 1.0,_f])
...
#                            else: P_BESS += P_step
                            ...
                            if Sinv_PV < Sbase_PV: Qlim_PV += Q_step # if Qbess is actually reduced, then compensate an amout from PV
...
#                            P_PV -= P_step
                            ...
                            if Sinv_BESS < Sbase_BESS: Qlim_BESS += Q_step
