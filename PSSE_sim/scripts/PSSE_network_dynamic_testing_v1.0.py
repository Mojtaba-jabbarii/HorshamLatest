...
simulation_batches=['S5255_NW','S52512_NW','S52513_NW']
...
def createModelCopy(origin_dir, target_dir):
#    target_dir=NEM_model_path+"\\"+scenario
#    origin_dir = main_folder+"\\base_model\\"+NEM_model
    try:
...
#def save_test_description(testInfoDir, scenario, scenario_params, ProfilesDict):
#    try:
...
#    standalone_script+='psspy.bsys(0,0,[0.0, 500.],0,[],0,[],0,[],0,[])\n'
    ...
    for root, dirs, files in os.walk(target_dir_DYR):
        for file_ in files:
            if '.dyr' in file_:
                psspy.dyre_add([_i,_i,_i,_i],os.path.join(root, file_),"","")   
                standalone_script+="psspy.dyre_add([_i,_i,_i,_i],'"+str('DYRs\\'+file_)+"', '','')\n"
    ...
    for root, dirs, files in os.walk(target_dir_DLL):
        for file_ in files:
            if '.dll' in file_:
                psspy.addmodellibrary(os.path.join(root, file_))  
                standalone_script+="psspy.addmodellibrary('"+str('DLLs\\'+file_)+"')\n"
                try:
    #                shutil.copyfile(zingen, model_copy_dir+'\\dsusr_zingen.dll')
                    shutil.copyfile(os.path.join(root, file_), target_dir_DLL+'\\'+file_) #copy dll file to the main folder for the standalone_script
                except:
                    pass
        ...
        if OutChansDict[inst]['Type'] == "VAR":
            if OutChansDict[inst]['Model'] == 'MachineArray':
                ...
                if OutChansDict[inst]['Model'] == PSSEmodelDict['PPC_model']:
                    ...
                    L = L_INV2
            try: psspy.var_channel([-1,L+OutChansDict[inst]['Position']],OutChansDict[inst]['Name'] )
            except: pass
...
#
#    ierr, L_PPC = psspy.cctmind_buso(273540,'SMAHYCF19','VAR')
#    ierr, K_PPC = psspy.cctmind_buso(273540,'SMAHYCF19','STATE')
#    ierr, L_INV = psspy.mdlind(273501,'1','GEN','VAR')
#    ierr, L_INV2 = psspy.mdlind(273502,'1','GEN','VAR')
#
#    try:
...
#    SUM_POC_DUM = 800009 # SUMSF DM 132kV
    ...
    psspy.voltage_channel([-1,-1,-1,CASN_132],r"""V_CASN_132""")
    psspy.voltage_channel([-1,-1,-1,ARM_132],r"""V_ARM_132""")
    psspy.voltage_channel([-1,-1,-1,SUM_POC],r"""V_SUM_POC""")  
...
if "OneDrive - OX2" in main_folder or "onedrive - ox2" in main_folder: # if the current folder is online (under OneDrive - OX2), create a new directory to store the result
    user = os.path.expanduser('~')
    if "OneDrive - OX2" in main_folder:
...
return_dict =  readtestinfo.readTestdef(testDefinitionDir+"\\"+TestDefinitionSheet, ['ProjectDetails', 'SimulationSettings', 'ModelDetailsPSSE', 'Setpoints', 'Profiles', 'NetworkScenarios','OutputChannels'])
...
def main():
    global main_folder, ModelCopyDir, ResultsDir, active_faults, reset_index, standalone_script, breakers, event_queue

    print(testRun)
    testRun_path = make_dir(ModelCopyDir,testRun) # crate a folder under model_copies to store the models to be included in the analysis
    error_file = open(testRun_path + '\\' + 'error_log.txt','a') #Text file to log errors
#    NEM_models=str(SimulationSettingsDict['model(s) for PSSE network studies']).split(",") # the name of the case files to be included in the analysis
    NEM_models = input_models_folder
    for NEM_model in NEM_models:  # loop through the case files
        NEM_model_path = make_dir(testRun_path,NEM_model)
        scenarios=NetworkScenDict.keys()
        scenarios.sort(key = lambda x: x[3:] )
        active_scenarios=[]
        for scenario in scenarios: # loop through all scenarios and find the active scenarios
        ...
        for scenario in active_scenarios: # loop through all active scenarios 
            event_list=NetworkScenDict[scenario]
            target_dir=NEM_model_path+"\\"+scenario
            origin_dir = main_folder+"\\base_model\\"+NEM_model
            createModelCopy(origin_dir, target_dir) # Create a model copy of the scenario to be analysed

            filenames = next(os.walk(target_dir), (None, None, []))[2] # not consider the sub folders.
            dll_files=[]
            for filename in filenames:
                if len(filename)>4:
#                    if(filename[-4:]==".sav"):
                    if(filename[-4:]==".sav") and (str(scenario) not in filename): #get the original case file
                        sav_file=filename
                    elif(filename[-4:]==".dll"):
                        dll_files.append(filename) # get all the dll files
                    elif(filename[-4:]==".dyr"):
                        dyr_file=filename
...
#           run network fault simulation on that copy, using channels specified in "ModelDetailsPSSE" and channels from Bus Lib and Branch Lib            
            ...
            event_queue=[]
#            dirname, flnm=os.path.split(psspy.sfiles()[0].rstrip(".sav"))
            psspy.case(target_dir+'\\'+sav_file) # Load the case
...
#            OutputDir =ResultsDir
            csv_file_path=ResultsDir+"\\"+testRun+"\\"+NEM_model+"\\"+scenario
            createPath(csv_file_path)
            csv_file_name = csv_file_path+"\\"+scenario+'_results'
            
            save_results(MYOUTFILE, csv_file_name) # save csv files for every run in properly organised folder structure. Save metadata about test itself in folder structure along with the result data
            scenario_params = NetworkScenDict[scenario]
            save_test_description(csv_file_path+"\\testInfo", scenario, scenario_params)
#            save_test_description(csv_file_path+"\\testInfo", scenario, scenario_params, ProfilesDict)
            import gc
            gc.collect()

    error_file_size = os.path.getsize(testRun_path + '\\' + 'error_log.txt')
