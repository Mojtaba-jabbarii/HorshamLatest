...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	!-----------------------------------------------------------------------------------------------
        ...
        ! pointers
        !----------------------------------------------------------------------
        MODULE SCKLIB_EXPORTS        ! Module holds all of the pointers/interfaces for accessing our DLL
        ...
        IMPLICIT NONE                   ! Implicit none will make debugging easier, by preventing complication of variables/types we did not declare.
            ...
            ABSTRACT INTERFACE                         					! We define an abstract interface as there is no concrete function behind it untill we load the DLL
				SUBROUTINE sma_sck_cinterface(xdata, xin, xout,&
    				& PCSFb, fdebugout, State)           				! We must provide what the function would look like if calling normally
				DOUBLE PRECISION,		INTENT(IN)	:: xdata(10)
				DOUBLE PRECISION,		INTENT(IN)	:: xin(50)
				DOUBLE PRECISION,		INTENT(OUT)	:: xout(50)			
				DOUBLE PRECISION,		INTENT(OUT)	:: PCSFb(10)
				DOUBLE PRECISION,		INTENT(OUT)	:: fdebugout(200)
				INTEGER,			INTENT(INOUT)	:: STATE							
				END SUBROUTINE sma_sck_cinterface                  		! End of the subroutine definition
            ...
            !------------------------------------------------------------   
            PROCEDURE(sma_sck_cinterface), POINTER :: SckLibPointer => NULL()		! Define pointer to a function that matches the previously defined interface
        ...
        END MODULE SCKLIB_EXPORTS      ! We have finished defining everything that we know about the DLL, we can end the module definition.

        !======================================================================
        ! SCKLIB_IMPORT_ROUTINES
        !----------------------------------------------------------------------
        ! This function will import the DLL. 
        ! Note: this is standard DLL importing. Not specific to this example
        !----------------------------------------------------------------------
        SUBROUTINE SCKLIB_IMPORT_ROUTINES        ! We need to define a routing that will load the DLL and extract the functions.
...
!
        ...
        USE KERNEL32      
#endif     
        USE SCKLIB_EXPORTS               ! Required for DLL function definitions, and function pointers
...
!
		...
		END INTERFACE
            ...
            !
            DLL_HANDLE = LoadLibrary(C_CHAR_'SMASC_K_100015R01.dll'//C_NULL_CHAR)
            ...
            CALL C_F_POINTER(TRANSFER(GetProcAddress(DLL_HANDLE, C_CHAR_'sma_sck_cinterface'//C_NULL_CHAR), C_NULL_PTR), SckLibPointer)			
#elif defined (__GFORTRAN__)	
			CALL C_F_PROCPOINTER(GetProcAddress(DLL_HANDLE, C_CHAR_'sma_sck_cinterface'//C_NULL_CHAR), SckLibPointer)
#endif
        ...
        SUBROUTINE SMA_SCK_FINTERFACE(xdata, xin, xout,&    				
			& PCSFb, fdebugout, State)  
        USE SCKLIB_EXPORTS ! Required for access to the DLL functions		
			DOUBLE PRECISION,		INTENT(IN)	:: xdata(10)
			DOUBLE PRECISION,		INTENT(IN)	:: xin(50)
			DOUBLE PRECISION,		INTENT(OUT)	:: xout(50) 			
			DOUBLE PRECISION,		INTENT(OUT)	:: PCSFb(10)
            ...
            IF ( DLL_HANDLE .EQ. 0 ) THEN
                 CALL SCKLIB_IMPORT_ROUTINES()
            ENDIF
        
            ! Call the dll Function
            !
            CALL SckLibPointer(xdata, xin, xout, PCSFb, fdebugout, STATE)
